...
export class BasicPipeline implements Pipeline {
    private _buildProcessTasks({ filters, quantizer, generators }: ProcessOptions): ProcessTasks {
        ...
        function createTask<Q>(stage: Stage<Q>, o: string | StageOptions): StageTask<Q> {
            let name: string,
                options: any
            if (typeof o === 'string') {
                name = o
    ...
    process(imageData: ImageData, opts: ProcessOptions): Bluebird<ProcessResult> {
        ...
        return this._filterColors(filters, imageData)
            .then((imageData) => this._generateColors(quantizer, imageData))
            .then((colors) => ({
                colors,
                palettes: this._generatePalettes(generators, colors)
            }))
            .props()
    }
    private _filterColors(filters: StageTask<Filter>[], imageData: ImageData) {
        return Bluebird.resolve(applyFilters(imageData, filters.map(({ fn }) => fn)))
    }
    private _generateColors(quantizer: StageTask<Quantizer>, imageData: ImageData) {
        return Bluebird.resolve(quantizer.fn(imageData.data, quantizer.options))

    }
    private _generatePalettes(generators: StageTask<Generator>[], colors: Swatch[]) {
        return Bluebird.props(
            generators.reduce((promises, { name, fn, options }) => {
                promises[name] = Bluebird.resolve(fn(colors, options))
                return promises
            }, <{ [name: string]: Bluebird<Palette> }>{})
        )
    }
}
