...
declare let CONTEXT: SampleContext;
...
class SampleView extends React.Component<SampleProps, SampleState> {
  ...
  private _onPalette(palette: Palette) {
    ...
    fetch("/palettes", {
      ...
      .catch((e) =>
        console.error(`Failed to send palette for '${this.props.name}': ${e}`)
      );
  }
  componentDidMount() {
  ...
  render() {
    ...
    return (
      <div>
        ...
        {Object.keys(palettes).map((source) => (
          <PaletteView
            key={source}
        ...
        {this.state.palette && (
          <PaletteView palette={this.state.palette} source="browser" />
        )}
      </div>
    );
  }
...
class App extends React.Component {
  render() {
    return (
      <div>
        {CONTEXT &&
          CONTEXT.current.map((sample) => (
            <SampleView key={sample.name} {...sample} />
          ))}
      </div>
    );
  }
