    ...
    return fs
      ...
      .then(function () {
        ...
        return promiseAllWait(move_promises).catch(function (err) {
          ...
          for (
            var i_moved_record = 0;
            i_moved_record < moved_records.length;
            i_moved_record++
          ) {
            undo_move_promises.push(
              fs.move(
                moved_records[i_moved_record].to,
                moved_records[i_moved_record].from
              )
            );
          }
      ...
      .then(function () {
        ...
        return rimraf(
          join(
            cwd,
            "../../.build/",
            pkgJSON.name.replace(/^@vibrant\//, "vibrant-")
          ),
          function (err) {
            if (err) {
              console.error(err);
